...
* connectivity $M_{\Phi} $ : the total number of non-zero entries in the matrices $ A_{l}, l \in \{{1, 2, \ldots, L} \}$, and the vectors $ b_{l}, l \in \{{1, 2, \ldots, L} \} $

* depth $L(\Phi) := L$

* width $W(\Phi) := max_{l=0,\ldots,L}N_{l}$

* weight manitude $B(\Phi) := max_{l=1,\ldots,L} max\{\| A_{l}\|_{\infty},\|b_{l}\|_{\infty}\}$
